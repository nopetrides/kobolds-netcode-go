    ...
    public partial class LeaningProcessor
    {
        ...
        internal void UpdateAngularVelocity()
        {
            if (customDetection == false)
            {
                ...
                if (UpsideDownSupport)
                    nowEulerY = (Quaternion.FromToRotation(Vector3.up, BaseTransform.up) * BaseTransform.rotation).eulerAngles.y;
        ...
        [Tooltip("Calculating side sway accordingly to the BaseTransform's up vector")]
        public bool UpsideDownSupport = false;
