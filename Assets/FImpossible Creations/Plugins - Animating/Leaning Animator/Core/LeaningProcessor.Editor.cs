...
namespace FIMSpace
{
    public partial class LeaningProcessor
    {
        ...
        public static LeaningAnimator firstDraw = null;
        ...
        public static void Editor_DrawTweakFullGUI( SerializedProperty sp_LeaningProcessor, ref LeaningAnimator.ELeaningEditorCat drawSetup, LeaningAnimator get = null )
        {
            if( get != null )
                ...
                {
                    firstDraw = get;
                    if( get.Parameters != null ) get.Parameters.unityanim = FTransformMethods.FindComponentInAllChildren<Animator>( get.Parameters.BaseTransform ? get.Parameters.BaseTransform : get.transform );
                }
            ...
            if( drawSetup == LeaningAnimator.ELeaningEditorCat.Setup )
            {
                ...
                EditorGUIUtility.labelWidth = 68; GUILayout.Space( 4 );
                ...
                if( get.Parameters.unityanim ) { GUI.enabled = false; GUILayout.Space( 4 ); EditorGUILayout.ObjectField( get.Parameters.unityanim, typeof( Animator ), true, GUILayout.MaxWidth( 70 ) ); GUI.enabled = true; }
                EditorGUILayout.EndHorizontal();
                ...
                if( _editor_DrawAuto )
                {
                    ...
                    if( !string.IsNullOrEmpty( sp_aParams.stringValue ) )
                    {
                        ...
                        {
                            if( get.Parameters.unityanim )
                            {
                                bool accelIsFloat = false;

                                for( int i = 0; i < get.Parameters.unityanim.parameterCount; i++ )
                                {
                                    if( get.Parameters.unityanim.GetParameter( i ).name == sp_aParams.stringValue )
                                    {
                                        if( get.Parameters.unityanim.GetParameter( i ).type == AnimatorControllerParameterType.Float ) accelIsFloat = true;
                                        break;
                                    }
                                }
                                ...
                                if( accelIsFloat )
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    GUILayout.Space( 6 );
                                    var accelThresh = sp_LeaningProcessor.FindPropertyRelative( "AccelValueThreshold" );
                                    float maxRange = Mathf.Ceil( accelThresh.floatValue );
                                    if( maxRange < 1f ) maxRange = 1f;
                                    EditorGUIUtility.labelWidth = 140;
                                    EditorGUIUtility.fieldWidth = 30;
                                    EditorGUILayout.Slider( accelThresh, 0f, maxRange + 0.0001f, new GUIContent( sp_aParams.stringValue + " Threshold:", accelThresh.tooltip ) );
                    ...
                    EditorGUILayout.PropertyField(sp_LeaningProcessor.FindPropertyRelative("UpsideDownSupport"));
