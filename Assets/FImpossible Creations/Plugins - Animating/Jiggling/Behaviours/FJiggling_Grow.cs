...
namespace FIMSpace.Jiggling
{
    ...
    public class FJiggling_Grow : FJiggling_Simple
    {
        ...
        {
            base.Init();
            if (TransformToAnimate.localScale == Vector3.zero) growProgress = 0f;
        }
        ...
        public virtual void StartGrowing(bool doRestart = false)
        {
            if (doRestart) growProgress = 0f;
            shrinking = false;
            StartJiggle();
        }
    ...
    public class FJiggling_GrowEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Animate Growing")) if (Application.isPlaying) targetScript.StartGrowing(true); else Debug.Log("You must be in playmode to run this method!");
