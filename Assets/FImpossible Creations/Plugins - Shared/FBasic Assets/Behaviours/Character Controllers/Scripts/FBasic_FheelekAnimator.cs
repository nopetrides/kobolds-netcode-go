    ...
    public class FBasic_FheelekAnimator
    {
        ...
        internal void Animate(float acc)
        {
            ...
            if (!AnimationHolder)
            {
                ...
                {
                    if (controller.CharacterRigidbody.linearVelocity.y < 0f)
                        if (Physics.Raycast(controller.transform.position, -controller.transform.up, 0.1f - controller.CharacterRigidbody.linearVelocity.y * Time.fixedDeltaTime))
                        {
                            Landing();
                        }
                }
        ...
        protected virtual void Landing()
        {
            ...
            if (controller.CharacterRigidbody.linearVelocity.y < -4.5f)
            {
