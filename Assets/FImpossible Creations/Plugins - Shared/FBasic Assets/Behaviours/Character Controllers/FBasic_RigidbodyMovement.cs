    ...
    public class FBasic_RigidbodyMovement : FBasic_CharacterHorizontalBase
    {
        ...
        protected override void FixedUpdate()
        {
            if (Grounded)
            {
                ...
                Vector3 newVelocity = new Vector3(lastTargetVelocityForward.x * accForw, CharacterRigidbody.linearVelocity.y, lastTargetVelocityForward.z * accForw);
            ...
            if (!Jumped)
            {
                if (!Grounded)
                {
                    ...
                    if (Physics.Raycast(groundRay, out groundHit, SkinHeight + 0.2f, ChracterLayerMask, QueryTriggerInteraction.Ignore))
                    {
                        ...
                        CharacterRigidbody.linearVelocity = new Vector3(CharacterRigidbody.linearVelocity.x, fakeYVelocity, CharacterRigidbody.linearVelocity.z);
