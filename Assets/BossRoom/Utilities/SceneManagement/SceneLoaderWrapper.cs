...
namespace Unity.Multiplayer.Samples.Utilities
{
	public class SceneLoaderWrapper : NetworkBehaviour
	{
		...
		private void OnNetworkingSessionStarted()
		{
			...
			if (!_mIsInitialized)
			{
				if (IsNetworkSceneManagementEnabled) NetworkManager.SceneManager.OnSceneEvent += OnSceneEvent;
		...
		public virtual void LoadScene(
			string sceneName, bool useNetworkSceneManager, LoadSceneMode loadSceneMode = LoadSceneMode.Single)
		{
		...
		{
			if (!IsSpawned || NetworkManager.ShutdownInProgress) m_ClientLoadingScreen.StopLoadingScreen();
		}
		...
		private void OnSceneEvent(SceneEvent sceneEvent)
		{
			switch (sceneEvent.SceneEventType)
			{
				...
				case SceneEventType.LoadEventCompleted: // Server told client that all clients finished loading a scene
					// Only executes on client or host
					if (NetworkManager.IsClient) m_ClientLoadingScreen.StopLoadingScreen();
					break;
