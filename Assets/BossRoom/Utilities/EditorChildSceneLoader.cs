...
/// <summary>
///     Allows setting a scene as a root scene and setting its child scenes. To use this, drag this component on any object
///     in a scene to make that scene a root scene. In the background, ChildSceneLoader will automatically manage this.
/// </summary>
	...
	public void ResetSceneSetupToConfig()
	{
		...
		foreach (var sceneAsset in sceneAssetsToLoad)
			sceneSetupToLoad.Add(
				new SceneSetup {path = AssetDatabase.GetAssetPath(sceneAsset), isActive = false, isLoaded = true});
...
public class ChildSceneLoaderInspectorGUI : Editor
{
	public override void OnInspectorGUI()
	{
		...
		if (GUILayout.Button("Save scene setup to config")) currentInspectorObject.SaveSceneSetup();

		if (GUILayout.Button("Reset scene setup from config...")) currentInspectorObject.ResetSceneSetupToConfig();
	}
}
...
public class ChildSceneLoader
{
	...
	private static void OnSceneLoaded(Scene _, OpenSceneMode mode)
	{
		...
		var scenesToLoadObjects = Object.FindObjectsByType<EditorChildSceneLoader>(sortMode: FindObjectsSortMode.None);
		if (scenesToLoadObjects.Length > 1) throw new Exception("Should only have one root scene at once loaded");
