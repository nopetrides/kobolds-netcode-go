...
namespace Kobolds
{
	...
	public class RagdollMover : FimpossibleComponent
	{
		...
		private void Start()
		{
			...
			_targetInstantRotation = transform.rotation;
		...
		protected virtual void Update()
		{
			...
			if (UpdateInput && updateMovement)
			{
				...
				var flatCamRot = Quaternion.Euler(0f, Camera.transform.eulerAngles.y, 0f);
		...
		private void FixedUpdate()
		{
			...
			{
				Rigb.position += transform.up * (_jumpRequest * 0.01f);
		...
		public void MoveTowards(Vector3 wPos, bool setDir = true)
		{
			...
			var mPos = new Vector3(transform.position.x, 0f, transform.position.z);
		...
		private bool DoRaycast()
		{
			if (RaycastRadius <= 0f)
				return Physics.Raycast(
					transform.position + transform.up, -transform.up,
					(IsGrounded ? 1.2f : 1.01f) + ExtraRaycastDistance, GroundMask, QueryTriggerInteraction.Ignore);

			return Physics.SphereCast(
				new Ray(transform.position + transform.up, -transform.up), RaycastRadius,
		...
		private void OnDrawGizmos()
		{
			...
			if (Mecanim)
			{
				...
				Gizmos.DrawRay(transform.position + transform.up, -transform.up * (IsGrounded ? 1.2f : 1.01f));
			}
		}
	}
}
