...
namespace Kobolds.Utils
{
	public static class KoboldUIUtils
	{
		...
		internal static void TransformUIDocumentWorldspace(
			UIDocument uiDocument, Camera camera, Transform worldspaceTransform, float yOffset = 0f)
		{
		...
		/// <param name="yOffset">Offset in Y from the position provided in worldspaceTransform</param>
		internal static void TranslateVeWorldToScreenspace(
			this VisualElement visualElement, Camera camera, Transform worldspaceTransform, float yOffset = 0f)
		{
			var positionInWorldSpace = new Vector3(
				worldspaceTransform.position.x, worldspaceTransform.position.y + yOffset,
				worldspaceTransform.position.z);
			...
			var panelSpacePosition = RuntimePanelUtils.ScreenToPanel(
				visualElement.panel, new Vector2(screenSpacePosition.x, Screen.height - screenSpacePosition.y));
