...
namespace MarchingCubesProject
{
    ...
    public class Example : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (smoothNormals)
            {
                // If using COMPACTCUBES, use the new averaged normal calculation
                if (mode == MARCHING_MODE.COMPACTCUBES && marching is CompactMarchingCubes compact)
                {
                    // ComputeAveragedNormals is not implemented; fallback to sampling normals per vertex
                ...
                {
                    // For CUBES and TETRAHEDRON, sample the normal at each vertex position
                    for (int i = 0; i < verts.Count; i++)
                    {
                        Vector3 p = verts[i];
                        float u = p.x / (width - 1.0f);
                ...
                // Create and load the normal renderer for visualization
