...
namespace MarchingCubesProject
{
    ...
    public class VoxelArray
    {
        ...
        {
            float dxP1 = GetVoxel(x + 1, y, z);
            float dyP1 = GetVoxel(x, y + 1, z);
            float dzP1 = GetVoxel(x, y, z + 1);

            float dxM1 = GetVoxel(x - 1, y, z);
            float dyM1 = GetVoxel(x, y - 1, z);
            float dzM1 = GetVoxel(x, y, z - 1);

            float dx = (dxP1 - dxM1) * 0.5f;
            float dy = (dyP1 - dyM1) * 0.5f;
            float dz = (dzP1 - dzM1) * 0.5f;
        ...
        public Vector3 GetFirstDerivative(float u, float v, float w)
        {
            ...
            float dxP1 = GetVoxel(u + hh, v, w);
            float dyP1 = GetVoxel(u, v + hh, w);
            float dzP1 = GetVoxel(u, v, w + hh);

            float dxM1 = GetVoxel(u - hh, v, w);
            float dyM1 = GetVoxel(u, v - hh, w);
            float dzM1 = GetVoxel(u, v, w - hh);

            float dx = (dxP1 - dxM1) * ih;
            float dy = (dyP1 - dyM1) * ih;
            float dz = (dzP1 - dzM1) * ih;
